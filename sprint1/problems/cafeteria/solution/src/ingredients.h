...
public:
    ...
    void StartFry(GasCooker& cooker, Handler handler) {
        ...
        frying_start_time_ = Clock::now(); // Запрещаем повторный вызов StartFry
    ...
    void StopFry() {
        ...
        gas_cooker_lock_.Unlock(); // Освобождаем горелку
    }
...
public:
    ...
    // Начинает приготовление хлеба на газовой плите. Как только горелка будет занята, вызовет handler
